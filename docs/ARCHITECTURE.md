# NavSim Local 架构设计文档

本文档详细介绍 NavSim Local 的架构设计、插件加载机制和关键设计决策。

## 📋 目录

- [系统架构](#系统架构)
- [三层解耦架构](#三层解耦架构)
- [插件加载机制](#插件加载机制)
- [数据流](#数据流)
- [关键设计决策](#关键设计决策)
- [性能考虑](#性能考虑)

## 🏗️ 系统架构

### 整体架构

```
┌─────────────────────────────────────────────────────────────┐
│                    navsim_local_debug                        │
│  (本地调试工具 - 独立运行，无需 navsim-online)                │
└─────────────────────────────────────────────────────────────┘
                            │
                            ↓
┌─────────────────────────────────────────────────────────────┐
│                    Plugin Framework                          │
│  (插件框架 - 动态加载、注册表、生命周期管理)                  │
│  - DynamicPluginLoader                                       │
│  - PlannerPluginRegistry                                     │
│  - PerceptionPluginRegistry                                  │
│  - PlannerPluginManager                                      │
│  - PerceptionPluginManager                                   │
└─────────────────────────────────────────────────────────────┘
                            │
                ┌───────────┴───────────┐
                ↓                       ↓
┌──────────────────────────┐  ┌──────────────────────────┐
│  Perception Plugins      │  │  Planner Plugins         │
│  (感知插件)              │  │  (规划器插件)            │
├──────────────────────────┤  ├──────────────────────────┤
│ GridMapBuilder           │  │ StraightLinePlanner      │
│ EsdfBuilder              │  │ AstarPlanner             │
│ ...                      │  │ JpsPlanner               │
└──────────────────────────┘  └──────────────────────────┘
```

### 核心组件

| 组件 | 职责 | 位置 |
|------|------|------|
| **Platform Layer** | 插件接口、数据结构、框架 | `platform/` |
| **Plugin Framework** | 插件加载、注册、管理 | `platform/src/plugin/` |
| **Plugins** | 具体算法实现 | `plugins/` |
| **Development Tools** | 脚手架、场景生成、测试 | `tools/` |
| **Applications** | 本地调试工具 | `apps/` |

## 🎯 三层解耦架构

### 架构图

```
┌─────────────────────────────────────────────────────────┐
│                    Platform Layer                        │
│  (平台层 - 插件框架、数据结构、接口定义)                  │
│                                                          │
│  核心接口:                                                │
│  - PlannerPluginInterface                                │
│  - PerceptionPluginInterface                             │
│                                                          │
│  数据结构:                                                │
│  - PlanningContext (输入)                                │
│  - PlanningResult (输出)                                 │
│  - OccupancyGrid, ESDFMap (感知数据)                     │
└─────────────────────────────────────────────────────────┘
                            ▲
                            │ 实现接口
                            │
┌─────────────────────────────────────────────────────────┐
│                    Adapter Layer                         │
│  (适配层 - 平台接口适配)                                  │
│                                                          │
│  职责:                                                    │
│  - 实现平台插件接口                                       │
│  - 数据格式转换 (平台 ↔ 算法)                            │
│  - 配置加载 (JSON → 算法参数)                            │
│  - 错误处理和日志                                         │
│                                                          │
│  示例:                                                    │
│  - AstarPlannerPlugin::plan()                            │
│  - GridMapAdapter (适配器模式)                           │
└─────────────────────────────────────────────────────────┘
                            ▲
                            │ 调用算法
                            │
┌─────────────────────────────────────────────────────────┐
│                   Algorithm Layer                        │
│  (算法层 - 纯算法实现)                                    │
│                                                          │
│  特点:                                                    │
│  - 只依赖 Eigen + STL                                    │
│  - 无平台依赖，可独立测试                                 │
│  - 可复用到其他项目                                       │
│  - 不处理 JSON                                           │
│                                                          │
│  示例:                                                    │
│  - AstarPlanner::plan(start, goal)                       │
│  - GridMapInterface (抽象接口)                           │
└─────────────────────────────────────────────────────────┘
```

### 层次职责

#### Platform Layer（平台层）

**职责**：
- 定义插件接口（`PlannerPluginInterface`, `PerceptionPluginInterface`）
- 提供标准化数据结构（`PlanningContext`, `PlanningResult`）
- 管理插件生命周期（加载、初始化、卸载）
- 提供插件注册表（单例模式）

**关键文件**：
- `platform/include/plugin/framework/planner_plugin_interface.hpp`
- `platform/include/plugin/framework/perception_plugin_interface.hpp`
- `platform/include/core/planning_context.hpp`
- `platform/include/plugin/data/planning_result.hpp`

#### Adapter Layer（适配层）

**职责**：
- 实现平台插件接口
- 数据格式转换（平台数据结构 ↔ 算法数据结构）
- 配置加载（JSON → 算法参数）
- 错误处理和日志输出

**示例**：
```cpp
// 适配器实现平台接口
class AstarPlannerPlugin : public navsim::plugin::PlannerPluginInterface {
public:
  bool plan(
      const navsim::planning::PlanningContext& context,  // 平台数据
      std::chrono::milliseconds deadline,
      navsim::plugin::PlanningResult& result) override {
    
    // 1. 提取数据
    Eigen::Vector3d start(context.ego.pose.x, context.ego.pose.y, context.ego.pose.yaw);
    Eigen::Vector3d goal(context.task.goal_pose.x, context.task.goal_pose.y, context.task.goal_pose.yaw);
    
    // 2. 调用算法
    auto algo_result = planner_->plan(start, goal);  // 算法数据
    
    // 3. 转换输出
    convertAlgorithmOutputToResult(algo_result, result);
    
    return result.success;
  }
};
```

#### Algorithm Layer（算法层）

**职责**：
- 纯算法实现
- 只依赖 Eigen 和 STL
- 无平台依赖
- 可独立编译和测试

**示例**：
```cpp
// 算法层只依赖 Eigen
class AstarPlanner {
public:
  Result plan(
      const Eigen::Vector3d& start,
      const Eigen::Vector3d& goal) {
    // 纯算法实现
    // 不依赖平台 API
    // 不处理 JSON
  }
};
```

## 🔌 插件加载机制

### 双重注册机制

为了同时支持**动态加载**和**静态链接**，系统采用双重注册机制：

#### 1. 动态注册（通过 dlsym）

```cpp
// adapter/register.cpp
extern "C" {
void registerAstarPlannerPlugin() {
  navsim::plugin::PlannerPluginRegistry::getInstance().registerPlugin(
      "AstarPlanner",
      []() -> std::shared_ptr<navsim::plugin::PlannerPluginInterface> {
        return std::make_shared<AstarPlannerPlugin>();
      });
}
}
```

**使用场景**：
- 用户开发的外部插件
- 通过 `--planner /path/to/plugin.so` 加载

#### 2. 静态注册（通过静态初始化器）

```cpp
// adapter/register.cpp
namespace {
struct AstarPlannerPluginRegistrar {
  AstarPlannerPluginRegistrar() {
    registerAstarPlannerPlugin();
  }
};
static AstarPlannerPluginRegistrar registrar;
}
```

**使用场景**：
- 内置插件
- 编译时静态链接到主程序

### 插件加载流程

```
1. 初始化插件系统
   └─> initializeAllPlugins()
       └─> loadAllBuiltinPlugins()  // 静态注册所有内置插件

2. 加载插件
   ├─> DynamicPluginLoader::loadPlugin("AstarPlanner", "")
   │   ├─> 查找插件库文件
   │   │   └─> findPluginLibrary("AstarPlanner")
   │   │       └─> "plugins/planning/astar_planner/libastar_planner_plugin.so"
   │   ├─> 动态加载库
   │   │   └─> dlopen(library_path, RTLD_NOW | RTLD_GLOBAL)
   │   └─> 调用注册函数
   │       └─> dlsym(handle, "registerAstarPlannerPlugin")
   │           └─> 注册到 PlannerPluginRegistry
   │
   └─> 插件已注册到注册表

3. 创建插件实例
   └─> PlannerPluginRegistry::getInstance().createPlugin("AstarPlanner")
       └─> 调用工厂函数创建实例
```

### 统一路径解析

支持两种加载方式：

#### 1. 短名称（推荐）

```bash
--planner AstarPlanner
```

系统自动查找：
1. `plugins/planning/astar_planner/libastar_planner_plugin.so`
2. `plugins/planning/libastar_planner_plugin.so`
3. `build/plugins/planning/astar_planner/libastar_planner_plugin.so`

#### 2. 完整路径

```bash
--planner /path/to/my_custom_planner/build/libmy_planner.so
```

直接加载指定路径的插件。

## 📊 数据流

### 规划流程

```
1. 加载场景
   └─> ScenarioLoader::loadFromFile("scenario.json")
       └─> PlanningContext

2. 运行感知插件（如果有）
   └─> PerceptionPluginManager::process(input, context)
       ├─> GridMapBuilder::process()
       │   └─> context.occupancy_grid = ...
       └─> EsdfBuilder::process()
           └─> context.esdf_map = ...

3. 运行规划器
   └─> PlannerPluginManager::plan(context, deadline, result)
       └─> AstarPlanner::plan(context, deadline, result)
           ├─> 检查依赖: context.occupancy_grid
           ├─> 创建适配器: GridMapAdapter
           ├─> 调用算法: planner_->plan(start, goal)
           └─> 转换输出: convertAlgorithmOutputToResult()

4. 输出结果
   └─> PlanningResult
       ├─> success
       ├─> trajectory
       ├─> computation_time_ms
       └─> failure_reason
```

### 数据结构

#### PlanningContext（输入）

```cpp
struct PlanningContext {
  EgoVehicle ego;                                    // 自车状态
  PlanningTask task;                                 // 规划任务
  std::unique_ptr<OccupancyGrid> occupancy_grid;     // 栅格地图（可选）
  std::unique_ptr<ESDFMap> esdf_map;                 // ESDF 地图（可选）
  std::unique_ptr<BEVObstacles> bev_obstacles;       // BEV 障碍物（可选）
  std::vector<DynamicObstacle> dynamic_obstacles;    // 动态障碍物
  double timestamp;                                  // 时间戳
};
```

#### PlanningResult（输出）

```cpp
struct PlanningResult {
  bool success;                                      // 是否成功
  std::string failure_reason;                        // 失败原因
  std::vector<TrajectoryPoint> trajectory;           // 轨迹
  double computation_time_ms;                        // 计算时间
  double total_cost;                                 // 总代价
  nlohmann::json debug_info;                         // 调试信息
};
```

## 🎨 关键设计决策

### 1. 为什么采用三层架构？

**问题**：如何在保持算法可复用性的同时，支持灵活的插件系统？

**解决方案**：三层解耦架构

**优点**：
- ✅ 算法层可独立测试和复用
- ✅ 适配层隔离平台变化
- ✅ 平台层提供统一接口

**示例**：
- 算法层的 `AstarPlanner` 可以在其他项目中使用
- 只需重新实现适配层即可

### 2. 为什么使用双重注册机制？

**问题**：如何同时支持内置插件（静态链接）和外部插件（动态加载）？

**解决方案**：双重注册机制

**优点**：
- ✅ 内置插件自动注册（静态初始化器）
- ✅ 外部插件动态加载（dlsym）
- ✅ 统一的注册表接口

### 3. 为什么感知插件和规划器分离？

**问题**：感知处理应该在规划器内部还是独立插件？

**解决方案**：独立的感知插件

**优点**：
- ✅ 单一职责原则（SRP）
- ✅ 感知算法可复用
- ✅ 规划器可声明依赖
- ✅ 灵活组合不同感知和规划算法

**示例**：
```cpp
// 规划器声明依赖
metadata.required_perception_data = {"occupancy_grid"};

// 平台自动加载 GridMapBuilder
// 或用户手动指定: --perception GridMapBuilder
```

### 4. 为什么使用 Eigen 而不是自定义数据结构？

**问题**：算法层应该使用什么数据结构？

**解决方案**：使用 Eigen

**优点**：
- ✅ 业界标准，广泛使用
- ✅ 高性能（SIMD 优化）
- ✅ 丰富的数学运算
- ✅ Header-only，易于集成

### 5. 为什么适配器中手动实现坐标转换？

**问题**：为什么不直接调用 `OccupancyGrid::worldToCell()`？

**原因**：动态加载的插件无法访问平台库函数

**解决方案**：在适配器中手动实现简单的转换逻辑

```cpp
// 不要这样（会导致链接错误）
auto [gx, gy] = grid->worldToCell({x, y});

// 应该这样（手动实现）
int gx = static_cast<int>((x - grid->config.origin.x) / grid->config.resolution);
int gy = static_cast<int>((y - grid->config.origin.y) / grid->config.resolution);
```

## ⚡ 性能考虑

### 1. 插件加载性能

- **动态加载开销**：~1-5 ms（首次加载）
- **静态链接开销**：0 ms（编译时链接）
- **建议**：内置插件使用静态链接，外部插件使用动态加载

### 2. 数据转换开销

- **Eigen ↔ 平台数据结构**：~0.01 ms（小规模数据）
- **优化**：使用 `std::move` 避免拷贝

### 3. 内存管理

- **智能指针**：使用 `std::unique_ptr` 和 `std::shared_ptr`
- **避免拷贝**：使用 `const&` 传递大对象
- **预分配**：预分配容器大小

### 4. 并发考虑

- **插件注册表**：线程安全（单例模式 + 互斥锁）
- **插件实例**：每个实例独立，无共享状态
- **建议**：多线程环境下为每个线程创建独立的插件实例

## 📚 参考资料

- [快速开始指南](GETTING_STARTED.md)
- [插件开发指南](PLUGIN_DEVELOPMENT.md)
- [开发工具指南](DEVELOPMENT_TOOLS.md)

## 🔍 深入阅读

### 插件系统实现

- `platform/src/plugin/framework/dynamic_plugin_loader.cpp` - 动态加载实现
- `platform/include/plugin/framework/plugin_registry.hpp` - 注册表实现
- `plugins/plugin_loader.cpp` - 内置插件加载

### 示例插件

- `plugins/planning/straight_line_planner/` - 最简单的示例
- `plugins/planning/astar_planner/` - 完整的搜索算法示例
- `plugins/perception/grid_map_builder/` - 感知插件示例

